...
def load_dataset(dataset: str = 'MNIST', datapath: str = './data/'):
    ...
    if dataset.__eq__('MNIST'):
        trainset = torchvision.datasets.MNIST(root=datapath, train=True, download=True, transform=mnist_transform)
        testset = torchvision.datasets.MNIST(root=datapath, train=False, download=True, transform=mnist_transform)

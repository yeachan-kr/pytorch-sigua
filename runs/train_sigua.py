...
def sigua_loss(model_loss, rt, bad_weight):
    ...
    _, good_and_bad_data_idx = torch.topk(model_loss, k=num_good_data + num_bad_data, largest=False)  # small loss samples
...
def train_step(data_loader, gpu: bool, model, optimizer, criterion, rt, bad_weight=0.001):
    global_step = 0
    avg_accuracy = 0.
    avg_loss = 0.
...
def train(FLAGS):
    ...
    if FLAGS.dataset.__eq__('TREC'):
        model = TextCNN(vocab=vocab.stoi, num_class=FLAGS.num_class, drop_rate=FLAGS.drop_rate,
